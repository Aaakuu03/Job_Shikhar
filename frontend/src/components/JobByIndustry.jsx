...
const JobByIndustry = () => {
  const { industry } = useParams(); // Get category from URL
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const response = await axios.get(
          `/api/jobseekers/jobs/industry/${industry.toUpperCase()}`
    ...
    fetchJobs();
  }, [industry]); // Dependency on category ensures jobs are fetched when category changes
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-center mb-8 text-[#2b2b2b]">
        {industry.toUpperCase()} Job Listings {/* Display the category name */}
      </h1>
